...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-3xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-secondary">Editar Usuario</h1>
        ...
        <div class="bg-white shadow p-6">
            <form action="{{ route('users.update', $user->id) }}" method="POST" id="userForm">
                ...
                <div class="mb-4">
                    <label for="password" class="block text-gray-700 text-sm font-bold mb-2">Nueva Contraseña <span
                            class="text-xs text-gray-500">(Dejar en blanco para mantener la actual)</span></label>
        ...
        @if ($user->id !== auth()->id())
        <div class="mt-6 bg-white shadow p-6 border-t-4 border-red-500">
            <h2 class="text-xl font-semibold text-red-600 mb-4 flex items-center">
                <i class="ri-delete-bin-line mr-1"></i> Eliminar Usuario
            </h2>
            <p class="mb-4 text-gray-700">
                La eliminación de una cuenta de usuario es permanente y eliminará todos los datos asociados.
            </p>
            <form action="{{ route('users.destroy', $user->id) }}" method="POST" class="inline"
                ...
                <button type="button" id="deleteButton"
                    class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 focus:outline-none flex items-center rounded-full">
                    <i class="ri-delete-bin-line mr-1"></i>Eliminar Usuario
                </button>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setupLoadingIndicator('deleteForm');

        // Configurar la confirmación de eliminación
        const deleteButton = document.getElementById('deleteButton');
        if (deleteButton) {
            deleteButton.addEventListener('click', function() {
                confirmDelete('{{ $user->name }}', document.getElementById('deleteForm'));
            });
        }
