...
<div class="container mx-auto px-4 py-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h1 class="text-2xl font-bold text-secondary">Editar Categoría</h1>
    ...
    <div class="bg-white shadow-md p-6">
        <form action="{{ route('news.categories.update', $category) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="4"
                    class="shadow rounded-2xl appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-primary @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
                @error('description')
                <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
