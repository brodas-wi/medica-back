...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <h1 class="text-2xl font-bold text-secondary">Nueva Noticia</h1>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-yellow-700">
                            Las noticias que crees serán revisadas por un administrador antes de ser publicadas. Puedes
                            guardarlas como borrador o enviarlas para revisión.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('news.articles.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <select name="status" id="status"
                            ...
                            @if (auth()->user()->hasAnyPermission(['auto_approve_news', 'review_news', 'manage_news']))
                                <option value="published" {{ old('status') == 'published' ? 'selected' : '' }}>Publicar
                                    ahora</option>
                                <option value="scheduled" {{ old('status') == 'scheduled' ? 'selected' : '' }}>Programar
                                    publicación</option>
                            @else
                                <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>Enviar para
                                    revisión</option>
                            @endif
                        </select>
                        @error('status')
                ...
                <div class="flex items-center justify-end">
                    <button type="button" id="submit-btn"
                        ...
                        <i class="ri-save-line mr-1"></i>Guardar Noticia
                    </button>
                </div>
            </form>
        </div>
...
@section('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Check if there's an old featured image value
            const oldFeaturedImage = document.getElementById('featured_image_path').value;
            if (oldFeaturedImage) {
                document.getElementById('image_preview').src = '/storage/' + oldFeaturedImage;
                document.getElementById('image_preview_container').classList.remove('hidden');
            }

            // Check if there's an old published_at value
            const oldPublishedAt = "{{ old('published_at') }}";
            if (oldPublishedAt) {
                const [date, time] = oldPublishedAt.split('T');
                if (date) document.getElementById('publish_date').value = date;
                if (time) document.getElementById('publish_time').value = time.substring(0, 5);
            }
        });
    </script>
@endsection
