...
<head>
    ...
    <title>{{ config('app.name', 'Sistema CMS') }} - Editor de Pie de Página</title>
...
<body class="font-sans antialiased bg-gray-100">
    ...
    <input type="hidden" id="page-id" value="{{ $footer->id ?? '' }}">
    <input type="hidden" id="page-load-url" value="{{ isset($footer) ? route('api.footers.load', $footer->id) : '' }}">
    <input type="hidden" id="page-store-url" value="{{ route('api.footers.store') }}">
    <input type="hidden" id="asset-upload-url" value="{{ route('api.assets.upload') }}">

    <!-- Scripts con Vite -->
    @vite(['resources/js/app.js', 'resources/js/footer-editor.js'])
